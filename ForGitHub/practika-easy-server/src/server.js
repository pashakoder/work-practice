...
const {readDataFromFile, writeDataToFile, upload, getNamesImgs, removeImg} = require('./utils');
...
app.post('/removeImg', (req, res) => {
  ...
  if (username == NICKNAME_ADMIN && password == PASSWORD_ADMIN) {
    removeImg(pngDel+'.png');
    res.status(200).send('Файл удален');
  } else {
    res.status(403).send('Неверные учетные данные');
  }
});
...
app.post('/admin_update', (req, res) => {
  ...
  if (username !== NICKNAME_ADMIN && password !== PASSWORD_ADMIN) {
      console.log('admin_update:', username, 'неудачная попытка');
   }
   writeDataToFile(USERS_FILE, data.users);
   writeDataToFile(GAMES_FILE, data.games);
   writeDataToFile(COMMENTS_FILE, data.comments)
   writeDataToFile(NEWS_FILE, data.news)
